            ...
            {sortedEvents.map((event) => (
              <div key={event.id} className="relative mb-8 pl-8 group">
                <div className={cn(
                  ...
                  event.type === 'AppointmentScheduled' ? 'bg-purple-500 border-purple-600' :
                ...
                <div className="rounded-lg border bg-card p-4 shadow-sm transition-all group-hover:shadow-md">
                  <p className="text-xs text-muted-foreground mb-1">
                    {isClient ? format(parseISO(event.date), "MMMM d, yyyy 'at' h:mm a") : 'Loading date...'} - <span className="font-medium">{event.type}</span>
                  </p>
